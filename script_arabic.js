...
document.getElementById('user-info-form').onsubmit = function(event) {
    ...
    const arabicNamePattern = /^[\u0600-\u06FF\s]{1,30}$/;
    if (!arabicNamePattern.test(nameInput)) {
        document.getElementById('name-error').textContent = 'الرجاء إدخال اسم باللغة العربية فقط (بحد أقصى 30 حرفًا)';
        isValid = false;
    }
    ...
    if (isNaN(age) || age < 5) {
        document.getElementById('age-error').textContent = 'الرجاء إدخال عمر صحيح (5 سنوات فما فوق)';
        isValid = false;
    }

    
    const arabicSchoolPattern = /^[\u0600-\u06FF\s]{1,50}$/; 
    if (!arabicSchoolPattern.test(schoolInput)) {
        document.getElementById('school-error').textContent = 'الرجاء إدخال اسم مؤسسة باللغة العربية فقط (بحد أقصى 50 حرفًا)';
        isValid = false;
    }
...
function displayQuestion() {
    ...
    content.innerHTML = `
        <h3 style="color:black;">${currentQuestionIndex}/${questions.length-1} : رقم السؤال</h3>
    ...
    <br><br>
        <h4><p>الوقت المتبقي: <span id="timer">15</span> ثانية</p></h4>
...
async function downloadCertificate() {
  ...
  try {
    ...
    if (pourcentage >= 0 && pourcentage < 20) {
      pdfPath = 'assets/pdf/0stars_arabic.pdf';
    } else if (pourcentage >= 20 && pourcentage < 60) {
      pdfPath = 'assets/pdf/1stars_arabic.pdf';
    } else if (pourcentage >= 60 && pourcentage < 100) {
      pdfPath = 'assets/pdf/2stars_arabic.pdf';
    } else {
    ...
    a.href = url;
    a.download = 'شهادة الفريق البيئي.pdf';
    ...
    console.error("لقد حدث خطأ:", error);
